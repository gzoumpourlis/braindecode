...
class EEGClassifier_weighted(NeuralNetClassifier):
	# doc = """..."""
	# __doc__ = update_estimator_docstring(NeuralNetClassifier, doc)

	def __init__(self, *args, criterion__reduce=False, cropped=False, callbacks=None,
				 iterator_train__shuffle=True, **kwargs):
		self.cropped = cropped
		callbacks = self._parse_callbacks(callbacks)
	...
	def get_loss(self, y_pred, y_true, *args, **kwargs):
		...
		loss_unreduced = NeuralNet.get_loss(self, y_pred, y_true, *args, **kwargs)
		...
		if training_flag:
			sample_weights = torch.from_numpy(np.array(self.sample_weight_list)).float().cuda()
			loss_reduced = (sample_weights * loss_unreduced).mean()
		else:
